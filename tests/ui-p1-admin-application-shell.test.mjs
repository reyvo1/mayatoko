...
test('UI-P1 primary navigation keeps canonical workspaces discoverable while permissions and UiSchema remain authoritative',()=>{assert.doesNotMatch(navigation,/enabledModuleCodes|activeModules/);assert.match(navigation,/identity\.permissions/);assert.match(navigation,/identity\.roles/);assert.match(navigation,/manifest\?\.uiSchemas/);assert.match(navigation,/override\?\.hidden !== true && hasPermission\(identity, workspace\)/);assert.match(page,/resolveAdminNavigation\(manifest, identity\)/);assert.doesNotMatch(page,/const NAV:/);});
